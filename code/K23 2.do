...
***
{
  *  one-time conversion of IPUMS shapefile (international.ipums.org/international/gis_yrspecific_2nd.shtml) to Stata format 
  ...
  scalar National_Average_FL = 129752482 / 330286606  // .39286956
  ...
  * district-level stats
  ...
  // Table 1: (dis)agreement between original and new program assignment variables
  ...
  * first row of rdplots
  ...
  rdplot K23_DPEP K23_Female_Literacy, p(2) nbins(18 22) graph_options(xscale(off fill) legend(off)  graphregion(margin(zero)) nodraw ylab(-.2(.2)1,format(%3.1f)) yscale(range(-.2 1)) name(DPEPOrig2, replace) title("Original, rescaled")) c(.39286)
  rdplot K23_DPEP K23_Female_Literacy, binselect(es)     graph_options(xscale(off fill) legend(off)  graphregion(margin(zero)) nodraw name(DPEPdefault, replace) title("Original, rdplot defaults") yscale(range(-.2 1) off fill)) c(.39286)            
  rdplot     DPEP     Female_Literacy, binselect(es)     graph_options(xscale(off fill) legend(off)  graphregion(margin(zero)) nodraw name(DPEPnew    , replace) title("New data, rdplot defaults") yscale(range(-.2 1)off fill)) c(`=National_Average_FL')
...
{
  PrepData
  rdplot K23_Years_Schooling K23_Female_Literacy if K23_Age<=35 & K23_Wages1   <. & abs(K23_Female_Literacy-.39286)<.2, nbins(6 7) p(2) c(.39286)                 graph_options(legend(off) xscale(off fill) graphregion(margin(2 0 0 0)) nodraw name(SchoolingOrig, replace))
  rdplot K23_Years_Schooling K23_Female_Literacy if K23_Age<=35 & K23_Wages1   <. & abs(K23_Female_Literacy-.39286)<.2, nbins(6 7) p(2) c(.39286)                 graph_options(legend(off) xlab(0(.2)1) xscale(off fill range(0 1)) ylab(6/12, format(%2.0f)) yscale(range(5.8 13)) graphregion(margin(zero)) nodraw name(SchoolingOrig2, replace))
  rdplot K23_Years_Schooling K23_Female_Literacy if K23_Age<=35 & K23_Wages1   <.                                     , binselect(es)   c(.39286)                 graph_options(legend(off) xscale(off fill) yscale(off fill) ylab(6/12, format(%2.0f)) yscale(range(5.8 13)) graphregion(margin(zero)) nodraw name(Schoolingdefault, replace))
  rdplot     Years_Schooling     Female_Literacy if     Age<=35 & lnWeekly_Wages<.                                     , binselect(es) c(`=National_Average_FL') graph_options(legend(off) xscale(off fill) yscale(off fill) ylab(6/12, format(%2.0f))yscale(range(5.8 13)) graphregion(margin(zero)) nodraw name(Schoolingnew, replace))
  ...
  rdplot K23_Wages K23_Female_Literacy if K23_Age<=35, binselect(es) c(.39286) graph_options(legend(off) graphregion(margin(0 2 0 0)) xtitle("Female literacy, 1991") ytitle(Wage earnings of under-35s (rupees/week)) xlab(0 "0%" .2 "20%" .4 "40%" .6 "60%" .8 "80%" 1 "100%") text(5000 .39286 "Eligibility threshold (39.29%)", place(se)))
  graph export output/Wagesdefault.png, height(1552) replace  // for blog
  rdplot Wages Female_Literacy if Age<=35, c(.39286) binselect(es) graph_options(legend(off) graphregion(margin(0 2 0 0)) xtitle("Female literacy, 1991") ytitle(Wage earnings of under-35s (rupees/week)) xlab(0 "0%" .2 "20%" .4 "40%" .6 "60%" .8 "80%" 1 "100%") text(5000 .39286 "Eligibility threshold (39.29%)", place(se)))
  ...
  rdplot K23_lnWages1       K23_Female_Literacy if K23_Age<=35 & abs(K23_Female_Literacy-.39286)<.2, nbins(8 8) p(2) c(.39286) graph_options(legend(off) graphregion(margin(zero)) xlab(0(.2)1) xscale(range(0 1)) ylab(6.3(.2)7.5) nodraw name(WagesOrig2, replace))
  rdplot K23_lnWages1       K23_Female_Literacy if K23_Age<=35                                     , binselect(es)   c(.39286) graph_options(legend(off) yscale(off fill) ylab(6.3(.2)7.5) graphregion(margin(2 0 0 0)) nodraw name(Wagesdefault, replace))
  rdplot     lnWeekly_Wages      Female_Literacy if     Age<=35                                    , binselect(es) c(`=National_Average_FL') graph_options(legend(off) graphregion(margin(zero)) ylab(6.3(.2)7.5) yscale(off fill) nodraw name(Wagesnew, replace))
  graph combine WagesOrig WagesOrig2 Wagesdefault Wagesnew, rows(1) l1title("Log wages, age≤35", size(small)) graphregion(margin(zero)) /*nodraw*/ name(Wages, replace)
...
* RDD table in text, saved to RDD.rtf
{
  ...
  forvalues s=1/2 {
    ...
    esttab OriginalSchooling`samplename' OriginalAutoSchooling`samplename' OriginalRobSchooling`samplename' OriginalClSchooling`samplename' OriginalWtSchooling`samplename' RevisedNo4Schooling`samplename' RevisedSchooling`samplename' AllNowtRobSchooling`samplename' AllRobSchooling`samplename' using output/RDD.rtf, append style(tab) keep(Conventional) varlabel(Conventional "Estimate") stat(h_l N_h, labels(Bandwidth Observations) fmt(%5.3g %8.0gc)) nonotes          nogaps star(* .1 ** .05 *** .01) nolines paren noobs `mtitles' fonttbl(\f0\fnil LM Roman 9;) msign("–") b(%5.3g) se(%5.3g)
    esttab OriginalWages`samplename'     OriginalAutoWages`samplename'     OriginalRobWages`samplename'     OriginalClWages`samplename'     OriginalWtWages`samplename'     RevisedNo4Wages`samplename'     RevisedWages`samplename'     AllNowtRobWages`samplename'     AllRobWages`samplename'     using output/RDD.rtf, append style(tab) keep(Conventional) varlabel(Conventional "Estimate") stat(h_l N_h, labels(Bandwidth Observations) fmt(%5.3g %8.0gc)) nonotes nonumber nogaps star(* .1 ** .05 *** .01) nolines paren noobs nomtitles fonttbl(\f0\fnil LM Roman 9;) msign("–") b(%5.3g) se(%5.3g)
    esttab OriginalFRDD`samplename'      OriginalAutoFRDD`samplename'      OriginalRobFRDD`samplename'      OriginalClFRDD`samplename'      OriginalWtFRDD`samplename'      RevisedNo4FRDD`samplename'      RevisedFRDD`samplename'      AllNowtRobFRDD`samplename'      AllRobFRDD`samplename'      using output/RDD.rtf, append style(tab) keep(Conventional) varlabel(Conventional "Estimate") stat(h_l N_h, labels(Bandwidth Observations) fmt(%5.3g %8.0gc)) nonotes nonumber nogaps star(* .1 ** .05 *** .01) nolines paren noobs nomtitles fonttbl(\f0\fnil LM Roman 9;) msign("–") b(%5.3g) se(%5.3g)
  }
}
...
{
preserve
collapse DPEP Female_Literacy if single_parent, by(DISTRICT_CODE)
...
{
cap erase output/validation.rtf
preserve
...
foreach c in `=National_Average_FL' .4 .41 {  // three cutoffs
  ...
  esttab using output/validation.rtf, append keep(Robust) cells(p(fmt(%3.2g))) nonotes nonumber nogaps nolines noobs mlabels() fonttbl(\f0\fnil LM Roman 9;)
}
restore
}
...
* sensitivity to threshold & bw
{
scalar clip = 3  // vertical graphing bound
foreach wt in "" WEIGHT {
  cwf default
  cap frame drop cutoff
  frame create cutoff str30 depvar double c tau lo hi
  forvalues s=1/4 {
    local title: word `s' of "DPEP treatment" Schooling "Log wages" "Return to schooling"
    local depvar: word `s' of DPEP Years_Schooling lnWages lnWages
    local fuzzy: word `s' of "" "" "" Years_Schooling
    local scalepar: word `s' of -1 -1 -1 1
    local clbc: word `s' of bc bc bc cl
    local clrb: word `s' of rb rb rb cl
    local vce: word `s' of "" "vce(cluster Female_Literacy)" "vce(cluster Female_Literacy)" "vce(cluster Female_Literacy)"
    preserve
    if `s'==1 collapse DPEP Female_Literacy (sum) WEIGHT, by(DISTRICT_CODE)
    forvalues c=.38(.001).42 {
      rdrobust `depvar' Female_Literacy, c(`c') `vce' fuzzy(`fuzzy') scalepar(`scalepar') weights(`wt')
      frame post cutoff ("`depvar'`fuzzy'") (`c') (cond(abs(e(tau_`clbc'))>clip,.,e(tau_`clbc'))) (cond(e(ci_r_`clrb')<-clip,.,max(-clip,e(ci_l_`clrb')))) (cond(e(ci_l_`clrb')>clip,.,min(clip,e(ci_r_`clrb'))))
    }
    restore
    cwf cutoff
    twoway rarea hi lo c, lwidth(none) fcolor(%50) || line tau c, lpat(solid) lwidth(vthin) legend(off) xline(`=National_Average_FL') || ///
      if depvar=="`depvar'`fuzzy'", name(cutoff`depvar'`fuzzy', replace) ylab(, notick format(%2.1f)) ///
      ytitle("") yline(0, lpat(solid) lcolor(gs8) lwidth(vthin)) plotregion(margin(zero)) nodraw ///
      `=cond(`s'<4, "xscale(off)", "ylab(-3/3)")' xlab(.38 .4 .41 .42 `=National_Average_FL', format(%6.5g) notick) xtitle("")
    cwf default
  }

  foreach c in `=National_Average_FL' .41 {
    local suffix = round(`c'*10000)
    cap frame drop bw
    frame create bw str30 depvar double h tau lo hi
    forvalues s=1/4 {
      ...
      preserve
      if `s'==1 collapse DPEP Female_Literacy (sum) WEIGHT, by(DISTRICT_CODE)
      qui rdrobust `depvar' Female_Literacy, c(`c') vce(cluster Female_Literacy) fuzzy(`fuzzy') scalepar(`scalepar') weights(`wt')
      local h_l = e(h_l)
      scalar r = e(b_l) / e(h_l)  // ratio of bc bw to estimation bw in default case
      forvalues h=.02(.001).15 {
        rdrobust `depvar' Female_Literacy, c(`c') h(`h') b(`=`h'*r') `vce' fuzzy(`fuzzy') scalepar(`scalepar') weights(`wt')
        frame post bw ("`depvar'`fuzzy'") (`h') (cond(abs(e(tau_`clbc'))>clip,.,e(tau_`clbc'))) (cond(e(ci_r_`clrb')<-clip,.,max(-clip,e(ci_l_`clrb')))) (cond(e(ci_l_`clrb')>clip,.,min(clip,e(ci_r_`clrb'))))
      }
      restore
      cwf bw
      twoway rarea hi lo h if hi<. & lo<., lwidth(none) fcolor(%50) || line tau h, lpat(solid) legend(off) || if depvar=="`depvar'`fuzzy'", name(bw`depvar'`fuzzy'`suffix', replace) ylab(, notick format(%3.1f)) ///
        yline(0, lpat(solid) lcolor(gs8) lwidth(vthin)) yscale(off fill) plotregion(margin(zero)) nodraw ///
        `=cond(`s'<4, "xscale(off)", "ylab(-3/3)")' xlab(.02(.01).15, notick labgap(zero)) xscale(range(.02 .15)) xtitle("") xline(`h_l')
      cwf default
    }
  }

  graph combine cutoffDPEP bwDPEP3929 bwDPEP4100, l1title(DPEP treatment, size(medium) margin(zero) bmargin(zero)) ycommon rows(1) imargin(zero) iscale(1.5) name(bwDPEP, replace) nodraw
  graph combine cutoffYears_Schooling bwYears_Schooling3929 bwYears_Schooling4100, l1title("Years of schooling", size(medium) margin(zero) bmargin(zero)) ycommon rows(1) imargin(zero) iscale(1.5) name(bwYears_Schooling, replace) nodraw
  graph combine cutofflnWages bwlnWages3929 bwlnWages4100, l1title("Log wages", size(medium) margin(zero) bmargin(zero)) ycommon rows(1) imargin(zero) iscale(1.5) name(bwlnWages, replace) nodraw
  graph combine cutofflnWagesYears_Schooling bwlnWagesYears_Schooling3929 bwlnWagesYears_Schooling4100, l1title(Return to schooling, size(medium) margin(zero) bmargin(zero)) rows(1) imargin(zero) iscale(1.5) name(bwlnWagesYears_Schooling, replace) nodraw
  graph combine bwDPEP bwYears_Schooling bwlnWages bwlnWagesYears_Schooling, cols(1) iscale(.5) name(bwsensitivity`wt', replace) ///
          t1title("  Bandwidth = MSE-optimized         Threshold = 39.29%               Threshold = 41%     ", ring(1) pos(12)) ///
          b1title("      Threshold                                        Bandwidth                                   Bandwidth", ring(1) pos(6) size(small)) graphregion(margin(zero))
  graph export output/sensitivity`wt'.png, height(2000) replace
}
}


* Versions of Khanna report Tables 1 & 2 with better inference: used in response to reports
{
cap erase output/KhannaReport.rtf
forvalues s=1/3 {
  local depvar: word `s' of Years_Schooling lnWages lnWages
  local fuzzy: word `s' of "" "" Years_Schooling
  ...
  esttab using output/KhannaReport.rtf, append keep(Robust) se stat(h_l N_h, labels(Bandwidth Observations) fmt(%5.3g %8.0gc)) nonotes nonumber nogaps nolines nomtitles star(* .1 ** .05 *** .01) msign("–") fonttbl(\f0\fnil LM Roman 9;)
}
}


* vary bandwidth in DID: used in response to reports
{
scalar clip = 3
PrepData
...
forvalues i=1/2 {
  ...
  forvalues s=1/3 {
    ...
    local indepvar: word `s' of 1.Young#1.IT 1.Young#1.IT "(Years_Schooling = 1.Young#1.IT)"
    cwf default
    cap frame drop DID
    frame create DID double bw b se
    forvalues bw=.075(.025).25 {
      ivreghdfe `depvar' `indepvar' if abs(Female_Literacy - National_Average_FL)<`bw', cluster(DISTRICT_CODE) a(Age DISTRICT_CODE)
...
esttab GEOrig GEAuto GERob GECl GEWt GERevNo4 GERev GEAll GEAllNew using output/GE.rtf, replace ///
  ...
  mtitles("Original" "Regression-specific BW" "Robust CI" "Clustered" "Sampling weights" "Revised data x 4 absent districts" "Revised data, all districts" "All changes" "Revised method & data")
